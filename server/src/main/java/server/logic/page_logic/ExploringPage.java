...
import shared.response.ExploringPageResponse;
...
public class ExploringPage extends Page{
    private static final Logger logger = LogManager.getLogger(ExploringPage.class);
    private LinkedList<long[]> tweetIDs;
    private long[] currentTweetID;

    public ExploringPage(Page previousPage, ClientHandler clientHandler) {
        super(PageType.exploring, previousPage, clientHandler);
    ...
    private void refresh(){
        ...
        long id; User user; Tweet tweet;
        int digitCount = String.valueOf(getLatestUserID()).length();
        StringBuilder stringID;
        int attemptCount=0;
        while(attemptCount<maxAttemptCount && tweetIDs.size()<maxTweetCount){
            stringID = new StringBuilder();
            for(int i=0;i<digitCount;i++) stringID.append(random.nextInt(10));
            id = Long.parseLong(stringID.toString());
            if(id<=getLatestUserID() && id>0){
                user = getUser(id);
                if(!currentUser.equals(user) && UserLogic.requestsToViewTweetsRandomly(user,currentUser.getId()) && user.getTweets().size()>0){
    ...
    private Page search(String text){
    ...
    public Page runCommand(ExploringPageRequest request) {
        ...
        else if(request.getCommand()==Command.search) return search(request.getStringArgs()[0]);
    ...
    public Response getData() {
        ...
        return new ExploringPageResponse(simpleTweet,dialog);
    }
}
